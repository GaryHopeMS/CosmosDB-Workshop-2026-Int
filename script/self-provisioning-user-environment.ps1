param(
  ...
  [string]$UserPrincipalName = (& az ad signed-in-user show --query userPrincipalName -o tsv 2>$null),
...
function Grant-VmManagedIdentityAccess {
  param(
    [Parameter(Mandatory = $true)][string]$SubscriptionId,
    [Parameter(Mandatory = $true)][string]$ResourceGroup,
    [Parameter(Mandatory = $true)][string]$VmName,
    [Parameter(Mandatory = $true)][bool]$SkipCosmos
  )
  ...
  Assert-LastAzCommand -FailureMessage "Failed to enable the managed identity on $VmName."
  ...
  $principalId = ([string](& az vm show `
    ...
    -o tsv)).Trim()
  Assert-LastAzCommand -FailureMessage "The managed identity principal ID could not be resolved for $VmName."
  if (-not $principalId) {
    throw "The managed identity principal ID could not be resolved for $VmName."
  }
  ...
  if (-not $existingContributorAssignment) {
    ...
    Assert-LastAzCommand -FailureMessage "Failed to grant Contributor on resource group $ResourceGroup."
  ...
  if (-not $SkipCosmos) {
    ...
    Assert-LastAzCommand -FailureMessage "Failed to list Cosmos DB accounts in $ResourceGroup."
    if ($cosmosAccounts.Count -eq 0) {
      throw "No Cosmos DB accounts found in $ResourceGroup."
    }
    ...
    foreach ($account in $cosmosAccounts) {
      ...
      if (-not $existingAssignment) {
        az cosmosdb sql role assignment create `
          ...
          --scope "/" `
          --only-show-errors | Out-Null
        Assert-LastAzCommand -FailureMessage "Failed to grant Cosmos DB Built-in Data Contributor on $($account.name)."
  ...
  $foundryName = ([string](& az cognitiveservices account list `
    ...
    -o tsv)).Trim()
  Assert-LastAzCommand -FailureMessage "Failed to list AIServices (Foundry) accounts in $ResourceGroup."
  if (-not $foundryName) {
    throw "No AIServices (Foundry) account found in $ResourceGroup."
  }
  ...
  foreach ($role in $foundryRoles) {
    ...
    if (-not $existingAssignment) {
      az role assignment create `
        ...
        --scope $foundryScope `
        --only-show-errors | Out-Null
      Assert-LastAzCommand -FailureMessage "Failed to grant $role on $foundryName."
  ...
  Write-Output "Managed identity access configured. Allow one to three minutes for role assignments to propagate."
}

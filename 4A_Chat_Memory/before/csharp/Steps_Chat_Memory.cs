// Lab 4A: Conversational History / Agent Memory - Consolidated Steps (STUDENT VERSION)
...
public class Steps_Chat_Memory
{
    ...
    {
        // STUDENT EXERCISE: replace the placeholder query below with a `c.sessionId = @sessionId`
        // query ordered by `c._ts DESC` with a LIMIT. See Instructions.md Step 3.
        var query = "SELECT * FROM c WHERE 1=0";
    ...
    private async Task<string> ChatAgent(string sid, string userMessage)
    {
        ...
        // STUDENT EXERCISE: replace the placeholder systemContent below with one that includes
        // baseSystemPrompt + retrieved contextText + chat history. See Instructions.md Step 4.
        var systemContent = $"{baseSystemPrompt} (placeholder - history and context not yet included)";
